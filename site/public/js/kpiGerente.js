        ...
        .then(function (resposta) {
            ...
            if (resposta.ok) {
                resposta.json().then(json => {
                    ...
                    for (let data = diaAtual; data <= 6; data++) {
                        if (data == 6) {
                            data = -1;
                            proximoDia = 0;
                        } else {
                        ...
                        for (let i = 0; i < listaDiasReinicializacao.length; i++) {
                            if (parseInt(listaDiasReinicializacao[i]) == proximoDia) {
                                let diaDaSemana;
                                if (proximoDia == 0) {
                                    diaDaSemana = 'Domingo';
                                } else if (proximoDia == 1) {
                                    diaDaSemana = 'Segunda-Feira';
                                } else if (proximoDia == 2) {
                                    diaDaSemana = 'Terça-Feira';
                                } else if (proximoDia == 3) {
                                    diaDaSemana = 'Quarta-Feira';
                                } else if (proximoDia == 4) {
                                    diaDaSemana = 'Quinta-Feira';
                                } else if (proximoDia == 5) {
                                    diaDaSemana = 'Sexta-Feira';
                                } else if (proximoDia == 6) {
                                    diaDaSemana = 'Sábado';
                                }

                                let diasAteReinicio = (proximoDia - diaAtual + 7) % 7;
                                let dataReinicio = hoje;
                                dataReinicio.setDate(hoje.getDate() + diasAteReinicio);

                                diaReinicializacao.innerHTML = diaDaSemana;
                                sessionStorage.DIAREINICIALIZACAO = diaDaSemana;

                                let horario = json[0].horaReinicializacao;
                                horario = json[0].horaReinicializacao
                                horarioReinicializacao.innerHTML = `${horario}`
                                sessionStorage.HORAREINICIALIZACAO = horario
                                horarioReinicializacao.innerHTML = `${dataReinicio.toLocaleDateString()} ${horario}`;
                                sessionStorage.HORAREINICIALIZACAO = `${dataReinicio.toLocaleDateString()} ${horario}`;
